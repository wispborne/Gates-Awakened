...
class ShowGateDestinationOptions : GateCommandPlugin() {
    ...
    override fun execute(ruleId: String?, dialog: InteractionDialogAPI?, params: MutableList<Misc.Token>?, memoryMap: MutableMap<String, MemoryAPI>?): Boolean {
        ...
        for ((index, gate) in activatedGates.take(maxcount).withIndex()) {
            ...
            addOption("Warp to ${gate.systemName} (${(fuelCostPerLY * gate.distanceFromPlayer).roundToInt()} fuel)", gate.systemId)
        }
